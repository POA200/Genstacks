...
import { getUploadPreSignedUrl, getDownloadPreSignedUrl } from './s3'; // Added for completeness
...
// API Helper Function (Remains unchanged)
...
    try {
        // FIX: Apply explicit stringify and ::jsonb casting to resolve the syntax error
...
            [
                JSON.stringify(layers), // FINAL DB FIX: Stringify the object
                supply, 
                collectionName, 
                jobId
            ]
...
            execArgv: ['-r', 'ts-node/register'], // CRITICAL FIX for Worker Threads
        });
        
        worker.on('error', (err) => { 
...
    const { jobId, layers, collectionName, supply } = req.body;
    
    if (!jobId) { return res.status(400).json({ status: 'error', message: 'Missing job ID.' }); }

    try {
        // FIX: Explicitly stringify the layers object for the query.
        const result = await pool.query(
            `UPDATE jobs SET 
             rarity_config = $1::jsonb, 
             collection_name = $2, 
             supply = $3,
             updated_at = NOW() 
             WHERE id = $4`,
...
        }
        
        return res.status(200).json({ status: 'saved', message: 'Progress saved.' });

    } catch (e) {
        console.error('Save progress error:', e);
        return res.status(500).json({ status: 'error', message: 'Database error during save.' });
    }
});
