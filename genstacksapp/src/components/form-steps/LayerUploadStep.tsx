...
const RENDER_API_URL = 'https://YOUR-RENDER-BACKEND.onrender.com/api'; 
...
const LayerUploadStep: React.FC<LayerUploadStepProps> = ({ onNext }) => {
  ...
  // CLEANUP 1: Removed updateLayerOrder as the logic uses setLayers directly
  ...
  const handleMoveLayer = (layerId: string, direction: 'up' | 'down') => {
    ...
    // CLEANUP 2: Removed unused const tempTargetOrder = targetOrder;
    
    // Perform the swap logic using setLayers action (which is why we don't need updateLayerOrder)
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fileList.forEach(file => {
        ...
        let traitName = parts[parts.length - 1]; // The file name
    ...
    try {
      ...
      const configResponse = await axios.post(`${RENDER_API_URL}/generate-s3-upload-config`, {
        jobId,
        collectionName,
        userId: stxAddress,
      ...
      // CLEANUP 3 & 4: Comment out the unused variables to silence the warning
      /* const uploadUrl = configResponse.data.uploadUrl; 
      const s3BaseKey = configResponse.data.s3BaseKey; */
