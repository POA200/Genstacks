...
    // The status check ensures the job is only started once
...
            setStatus('FAILED');
            // The 404 from Render is fixed by the backend deployment, but error handling remains
...
    };
  }, [stxAddress, status]); // Added status to dependency array for safety
            ...
            case 'GENERATING':
                return (
                    <span className="text-yellow-500 flex items-center">
                        <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Generating {supply} NFTs...
                    </span>
                );
            case 'COMPLETE':
                return (
